...
impl Game{
    ...
    fn alter_chemin(&self, chemin: &mut Vec<Vec<i32>>,r: i32, c: i32, valeur_courante: i32) {
        if r != -1 {
            if self.carte[r as usize][c as usize]!=Cell::Mur && 
                (chemin[r as usize][c as usize]==-1 || valeur_courante+1<chemin[r as usize][c as usize]){
    ...
    pub fn calcul_where_to_go(& mut self) -> String{
        ...
        let mut chemin: Vec<Vec<i32>> = vec![vec![-1;self.size_col as usize];self.size_row as usize];
        let mut cherche: Vec<Vec<i32>> = vec![vec![-2;self.size_col as usize];self.size_row as usize];
        if !self.control_present{
            ...
            for i in 0..self.size_row as usize{
                ...
                for j in 0..self.size_col as usize{
                    if cherche[i][j]==-2{
                        let mut val_min = self.get_val_min(i, j, &mut cherche);
                        if val_min>=0{
        ...
        if self.find_control{
            // back to T 
            if val_n != -1 && (val_n<=val_e || val_e==-1) && (val_n<=val_s || val_s==-1)&& (val_n<=val_w||val_w==-1){
                ret = String::from("UP");
            }else if val_s!=-1 && (val_s<=val_n|| val_n==-1)  && (val_s<=val_e || val_e==-1) && (val_s<=val_w|| val_w==-1) {
                ret = String::from("DOWN");
            }else if val_e!=-1 && (val_e<=val_n || val_n==-1) && (val_e<=val_s || val_s==-1) && (val_e<=val_w|| val_w==-1) {
                ret = String::from("LEFT");
            }else if val_w!=-1 && (val_w<=val_n || val_n==-1) && (val_w<=val_s || val_s==-1) && (val_w<=val_e|| val_e==-1) {
...
mod tests {
    ...
    fn test_line_tab() {
        let line = ".TC?#";
        let a = [Cell::Vide,Cell::Base,Cell::Control,Cell::Unknown,Cell::Mur];
    ...
    fn test_reelle(){
        ...
        game.add_line(init_ligne_cell(&String::from("???############???????????????")));
        game.add_line(init_ligne_cell(&String::from("???############???????????????")));
        game.add_line(init_ligne_cell(&String::from("???##T......C#????????????????")));
        game.add_line(init_ligne_cell(&String::from("???############???????????????")));
        game.add_line(init_ligne_cell(&String::from("???############???????????????")));
        game.add_line(init_ligne_cell(&String::from("??????????????????????????????")));
        game.add_line(init_ligne_cell(&String::from("??????????????????????????????")));
